...
def mock_document_reference():
    ...
    doc_ref.version = "1"
    doc_ref._build_s3_location = Mock(return_value="s3://test-lg-bucket/9000000001/test-doc-id")
    return doc_ref
...
def mock_virus_scan_service(
    ...
    mock = mocker.patch("services.upload_document_reference_service.get_virus_scan_service")
    yield mock
...
def test_handle_upload_document_reference_request_success(service, mock_document_reference, mocker):
    ...
    service.document_service.fetch_documents_from_table.side_effect = [[mock_document_reference], [mock_document_reference2]]
...
def test_handle_upload_document_reference_request_with_exception(service):
    ...
    service.document_service.fetch_documents_from_table.side_effect = Exception("Test error")
...
def test_fetch_preliminary_document_reference_success(service, mock_document_reference):
    ...
    service.document_service.fetch_documents_from_table.return_value = [mock_document_reference]
...
def test_fetch_preliminary_document_reference_multiple_documents_warning(service, mock_document_reference):
...
def test_fetch_preliminary_document_reference_exception(service):
    ...
    document_key = "test-doc-id"
    service.document_service.fetch_documents_from_table.side_effect = (ClientError({"error": "test error message"}, "test"),)

    with pytest.raises(DocumentServiceException):
        service._fetch_preliminary_document_reference(document_key)


def test__process_preliminary_document_reference_clean_virus_scan(service, mock_document_reference, mocker):
    """Test processing document reference with a clean virus scan"""
    object_key = "staging/test-doc-id"

    mocker.patch.object(service, "_perform_virus_scan", return_value=VirusScanResult.CLEAN)
    mock_process_clean = mocker.patch.object(service, "_process_clean_document")
    mock_finalize_transaction = mocker.patch.object(service, "_finalize_and_supersede_with_transaction")
...
def test__process_preliminary_document_reference_infected_virus_scan(service, mock_document_reference, mocker):
    """Test processing document reference with an infected virus scan"""
    object_key = "staging/test-doc-id"

    mocker.patch.object(service, "_perform_virus_scan", return_value=VirusScanResult.INFECTED)
    mock_process_clean = mocker.patch.object(service, "_process_clean_document")
...
def test_perform_virus_scan_exception_returns_infected(service, mock_document_reference, mocker):
...
def test_process_clean_document_exception_restores_original_values(service, mock_document_reference, mocker):
    ...
    mocker.patch.object(service, "copy_files_from_staging_bucket", side_effect=Exception("Copy failed"))
...
def test_copy_files_from_staging_bucket_success(service, mock_document_reference):
    ...
    expected_dest_key = f"{mock_document_reference.nhs_number}/{mock_document_reference.id}"
...
def test_delete_file_from_staging_bucket_success(service):
    ...
    service.s3_service.delete_object.assert_called_once_with(MOCK_STAGING_STORE_BUCKET, source_file_key)
...
@pytest.mark.parametrize(
    "object_key,expected_document_key,expected_nhs_number",
    [
        ("staging/documents/test-doc-123", "test-doc-123", "9000000001"),
        ("folder/subfolder/another-doc", "another-doc", "9000000001"),
        ("simple-doc", "simple-doc", "9000000001"),
    ],
)
def test_document_key_extraction_from_object_key(
    service, mock_document_reference, object_key, expected_document_key, expected_nhs_number
):
    ...
    service.document_service.fetch_documents_from_table.side_effect = [[mock_document_reference], []]
...
def test_finalize_and_supersede_with_transaction_with_existing_finals(service, mock_document_reference, mocker):
...
def test_finalize_and_supersede_with_transaction_no_existing_docs(service, mock_document_reference, mocker):
...
def test_finalize_and_supersede_with_transaction_multiple_existing(service, mock_document_reference, mocker):
...
def test_finalize_and_supersede_with_transaction_skips_same_id(service, mock_document_reference, mocker):
...
def test_finalize_and_supersede_with_transaction_handles_transaction_cancelled(service, mock_document_reference, mocker):
    ...
    transaction_error = ClientError(
        error_response={"Error": {"Code": "TransactionCanceledException"}}, operation_name="TransactWriteItems"
    )
...
def test_process_preliminary_document_reference_exception_during_processing(service, mock_document_reference, mocker):
    """Test that exceptions during processing are properly raised"""
    object_key = "staging/test-doc-id"

    mocker.patch.object(service, "_perform_virus_scan", return_value=VirusScanResult.CLEAN)
    mocker.patch.object(service, "_process_clean_document", side_effect=Exception("Processing failed"))
